...
class SudokuSearchProblem:
    ...
    def getStartState(self):
        ...
        # display(self.start_values)
        return self.start_values
        # return parse_grid(self.start_values)
    ...
    def isGoalState(self, state):
        ...
        #     # print "value ->", value
        #     value_for_key = value
        #     peers_for_key = peers[key]
        #     for square_unit in peers_for_key:
        #         if state[square_unit] == value:
        #             return False
    ...
    def getSuccessors(self, state):
        ...
        successors = []
        length, unit_cell = min((len(state[s]), s) for s in squares if len(state[s]) > 1)

        for no in state[unit_cell]:
            copy_state = state.copy()
            new_values = assign(copy_state, unit_cell, no)
            if new_values == False:
                continue
            else:
                successors.append([new_values, [unit_cell, no], 1])
        ## YOUR CODE HERE
        # util.raiseNotDefined()
